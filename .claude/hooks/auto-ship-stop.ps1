...
$reason = "You have unshipped work on branch '$branch' (uncommitted changes and/or commits not yet pushed to origin). Per CLAUDE.md's Automation & Approval Policy, commit / push / PR creation / merge-to-main are pre-authorized for routine feature or fix work -- do not stop to ask for confirmation. Run the full ship workflow now, in order, stopping only if a step fails: (1) mvn --batch-mode clean verify; (2) /code-review at medium effort against the working diff, fixing any CONFIRMED correctness finding and re-running step 1; (3) stage only the relevant files and commit with a Conventional Commit message; (4) git push (with -u if there is no upstream yet) -- if the push is rejected as diverged, fetch and reconcile per ship.md's guarded-auto-resolve rule: only auto-resolve a conflict if it is purely a stale literal/test-value mismatch where the local side is this branch's own newer intentional change, verify with grep and re-run step 1 before committing the merge, and stop to ask about anything else (logic, imports, structure, or ambiguous intent); (5) check for an existing PR with 'gh pr view --json number,state', and if none exists, create one with 'gh pr create --fill --base main'; (6) run 'gh pr merge --auto --squash'; (7) report the PR URL and merge status."
